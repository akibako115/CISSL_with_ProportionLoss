...
def train(
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            pseudo_orig[idx_u, :] = p.data.cpu().float()
            ...
            if args.darp and epoch > args.warm:
                if batch_idx % args.num_iter == 0:
                    ...
                    targets_u, weights_u = estimate_pseudo(U_SAMPLES_PER_CLASS, pseudo_orig, args.num_class, args.alpha)
                    ...
                    # Select
                    targets_u = opt_res[idx_u].detach().to(args.device)
                    pseudo_orig = pseudo_orig_backup
                else:
                    # Using previously saved pseudo-labels
                    targets_u = pseudo_refine[idx_u].to(args.device)
        ...
        with torch.amp.autocast(device_type=args.device.type, dtype=torch.float16):
