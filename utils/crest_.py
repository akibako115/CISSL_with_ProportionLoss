...
def crest_select(args, model_ema, u_dataset, labeled_dataset, num_classes: int, t_cur: float = None):
    ...
    t_cur: Progressive DA の現在温度（None時は args.crest_t_min を使用）
    """
    model_ema.eval()

    # 1) μ を計算（鏡映ランク）
    labeled_counts = labeled_hist_from_dataset(labeled_dataset, num_classes)
    rates = compute_sampling_rates_from_labeled_counts(labeled_counts, alpha=getattr(args, "crest_alpha", 1/3))
    ...
    loader = data.DataLoader(u_dataset, batch_size=args.batch_size*args.mu, shuffle=False, 
                            drop_last=False, num_workers=min(16, os.cpu_count() or 8), 
    ...
    for batch in loader:
        ...
        for j in range(len(idx_local)):
            if conf[j] >= tau:
                c = int(pred[j])
                class_to_cands[c].append((idx_local[j], conf[j], c))
    ...
    for c in range(num_classes):
        ...
        if k <= 0:
            continue
        pick = cand[:k]
        chosen_idx.extend([p[0] for p in pick])
        chosen_lbl.extend([p[2] for p in pick])
        print(f"Class {c}: {len(cand)} candidates, rate={rates[c]:.3f}, selected={k}")
